# NN COLA Emulator for wCDM, 25 refs
import os
...
def load_models():
    ...
    for i, z in enumerate(cola_redshifts):
        nn_models.append(keras.models.load_model(f"{path_to_emulator}/models/wCDM_25REFS/NN_Z{z:.3f}"))
    return nn_models
...
def load_pcas():
    ...
    for i, z in enumerate(cola_redshifts):
        avgs[i] = np.loadtxt(f"{path_to_emulator}/data/averages_wcdm_25refs/avg_{i}.txt")
        pcs[i] = np.loadtxt(f"{path_to_emulator}/data/pc_basis_wcdm_25refs/pcs_{i}.txt")
    return pcs, avgs
...
def load_normalization_factors():
    ...
    """
    mins = np.loadtxt(f"{path_to_emulator}/data/mins_wcdm_25refs.txt")
    maxs = np.loadtxt(f"{path_to_emulator}/data/maxs_wcdm_25refs.txt")
    return mins, maxs
